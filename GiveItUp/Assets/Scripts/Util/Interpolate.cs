 ...
 */
    ...
    */
     ...
     * using Catmull-Rom.
     */
    public static IEnumerable<Vector3> NewCatmullRom(Transform[] nodes, int sliceDist, bool loop) {
        return NewCatmullRom<Transform>(nodes, TransformDotPosition, sliceDist, loop);
    }
     ...
     */
    public static IEnumerable<Vector3> NewCatmullRom(Vector3[] points, int sliceDist, bool loop) {
        return NewCatmullRom<Vector3>(points, Identity, sliceDist, loop);
    }
     ...
     * function directly.
     */
    static IEnumerable<Vector3> NewCatmullRom<T>(IList nodes, ToVector3<T> toVector3, int sliceDist, bool loop) {
        // need at least two nodes to spline between
        if (nodes.Count >= 2) {
            ...
            for (int current = 0; loop || current < last; current++) {
                ...
                int stepCount = Mathf.RoundToInt(Vector3.Distance(toVector3((T)nodes[start]), toVector3((T)nodes[end])) / sliceDist) + 1;
