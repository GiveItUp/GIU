...
public class SpriteSizer : ScriptableWizard 
{
	...
	List<SpriteRoot> sprites = new List<SpriteRoot>();
	...
	void ScanProjectFolder(List<SpriteRoot> sprites)
	{
		string[] files;
		GameObject obj;
		SpriteRoot[] c;
		...
		while (stack.Count > 0)
		{
			...
			try
			{
				...
				for (int i = 0; i < files.Length; ++i)
				{
					...
					if (obj != null)
					{
						c = obj.GetComponentsInChildren<SpriteRoot>(true);
