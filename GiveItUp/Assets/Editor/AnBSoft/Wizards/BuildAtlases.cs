...
public class MaterialSpriteList
{
	...
	public List<ISpriteAggregator> sprites = new List<ISpriteAggregator>();
}
...
public class AtlasTextureList
{
	...
	public List<Texture2D> textures = new List<Texture2D>();	// The source textures for the atlas
	public List<Texture2D> trimmedTextures = new List<Texture2D>();	// Trimmed versions of the source textures, so our atlas will be optimized
	public Rect[] uvs;			// UVs of the trimmed textures, once placed on the atlas
	public List<SPRITE_FRAME> frames = new List<SPRITE_FRAME>();	// Frame info for each trimmed textures (offset, etc)
...
public class BuildAtlases : ScriptableWizard
{
	...
	void OnWizardCreate()
	{
		List<ISpriteAggregator> sprites = new List<ISpriteAggregator>();
	...
	protected Object[] FindAllOfType(System.Type t)
	{
		...
		Object[] os = Resources.FindObjectsOfTypeAll(t);
#else
		Object[] os = FindObjectsOfTypeAll(t);
#endif
		foreach (Object o in os) 
		{
			...
			string assetPath = AssetDatabase.GetAssetPath(((Component)o).transform.root.gameObject);
	...
	void FindPackableSprites(List<ISpriteAggregator> sprites)
	{
		List<Object> objList = new List<Object>();

		// Get all packed sprites in the scene:
		Object[] o = FindAllOfType(typeof(AutoSpriteBase));
		...
		for (int i = 0; i < objList.Count; ++i)
		{
...
#endif
			{
				...
				if (PrefabType.PrefabInstance != PrefabUtility.GetPrefabType(objList[i]))
					sprites.Add((ISpriteAggregator)objList[i]);
	#else
	...
	void ScanProjectFolder(List<ISpriteAggregator> sprites)
	{
		string[] files;
		GameObject obj;
		Component[] c;
				...
				for (int i = 0; i < files.Length; ++i)
				{
					...
					if (obj != null)
					{
						...
						foreach (Component comp in c)
						{
							...
							{
								sprites.Add((ISpriteAggregator)comp);
							}
						}
					}
				}
	...
	// MaterialSpriteLists:
	List<MaterialSpriteList> OrganizeByMaterial(List<ISpriteAggregator> sprites)
	{
		string errString = "";
