...
public class SpriteFont
{
	...
	/// </summary>
	/// <param name="str">The string to measure.</param>
	/// <param name="spacingCoef">The character spacing coefficient.</param>
	/// <returns>The width, in pixels, of the string.</returns>
	public float GetWidth(string str, float spacingCoef)
	{
		...
		if (chr != null)
			width = chr.UVs.width * (float)texWidth;
		...
		for (int i = 1; i < str.Length; ++i)
		{
			...
			if (chr != null)
				width += (chr.xAdvance + chr.GetKerning(str[i - 1])) * spacingCoef;
		}
	...
	{
		return GetWidth(str, start, end, 1f);
	}
	...
	{
		return GetWidth(sb, start, end, 1f);
	}
	...
	public float GetWidth(StringBuilder sb, int start, int end, float spacingCoef)
	{
		...
		for (int i = start + 1; i <= end; ++i)
		{
			...
			if (chr != null)
				width += (chr.xAdvance + chr.GetKerning(sb[i - 1])) * spacingCoef;
		}
