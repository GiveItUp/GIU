...
- (void)sendContentRequestWithContentUnitID:(NSString*)contentUnitID
					...
					showsOverlayImmediately:(BOOL)showsOverlayImmediately
						 	 shouldAnimate:(BOOL)shouldAnimate dimensions:(NSDictionary *)dimensions
{
...
- (void)request:(PHPublisherContentRequest*)request contentDidFailWithError:(NSError*)error
{
	...
	switch( (UpsightRequestType)request.hashCode )
	{
		...
		default:
		{
			NSLog( @"request:contentDidFailWithError: received an unknown requestType: %i", request.hashCode );
		}
	}
}
