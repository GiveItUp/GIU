...
public abstract class AutoSpriteControlBase : AutoSpriteBase, IControl, IUIObject, IPackableControl
{
	...
	public override void Hide(bool tf)
	{
		...
		if (!IsHidden() && tf)
		{
			if (GetComponent<Collider>() is BoxCollider && Application.isPlaying)
			{
				savedColliderSize = ((BoxCollider)GetComponent<Collider>()).size;
		...
		else if (IsHidden() && !tf)
		{
			...
			{
				((BoxCollider)GetComponent<Collider>()).size = savedColliderSize;
			}
		}
	...
	public virtual void Copy(SpriteRoot s, ControlCopyFlags flags)
	{
		...
		if ((flags & ControlCopyFlags.Appearance) == ControlCopyFlags.Appearance)
		{
			...
			if (c.GetComponent<Collider>() != null)
			{
				...
				{
					gameObject.AddComponent(c.GetComponent<Collider>().GetType());
					customCollider = c.customCollider;
				}

				if (GetComponent<Collider>().GetType() == c.GetComponent<Collider>().GetType())
				{
					if (c.GetComponent<Collider>() is BoxCollider)
					{
						if (GetComponent<Collider>() == null)
					...
					else if (c.GetComponent<Collider>() is SphereCollider)
					{
						if (GetComponent<Collider>() == null)
					...
					else if (c.GetComponent<Collider>() is MeshCollider)
					{
						if (GetComponent<Collider>() == null)
					...
					else if (c.GetComponent<Collider>() is CapsuleCollider)
					{
						if (GetComponent<Collider>() == null)
	...
	public virtual void UpdateCollider()
	{
		...
		if (!(GetComponent<Collider>() is BoxCollider) || IsHidden() || m_spriteMesh == null || customCollider)
			return;
