...
public class UIScrollList : MonoBehaviour, IUIObject
{
	...
	/// <summary>
	/// The number of items to be destroyed per-frame while processing 
	/// a clear queue (queue of items to be destroyed following the
	/// clearing of a list).
	/// </summary>
	...
	protected Queue<IUIListObject> clearQueue = new Queue<IUIListObject>(); // The list of items pending destruction
	...
	protected void Start()
	{
		...
		if (GetComponent<Collider>() == null && touchScroll)
		{
	...
	public void SetupCameraAndSizes()
	{
		...
		if (touchScroll)
		{
			...
			if (GetComponent<Collider>() == null)
				...
				bc = (BoxCollider)GetComponent<Collider>();
	...
	// Processes the queue of items to be destroyed following the 
	// clearing of a list, spread out over multiple frames.
	protected IEnumerator ProcessClearQueue()
	{
		while (clearQueue.Count > 0)
		{
			Debug.Log(Time.frameCount);
			for (int count = 0; count < clearChunkSize && clearQueue.Count > 0; ++count)
			{
				IUIListObject obj = clearQueue.Dequeue();
